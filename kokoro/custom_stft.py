...
class CustomSTFT(tf.keras.layers.Layer):
    ...
    def inverse(self, magnitude, phase):
        ...
        istft_real = tf.nn.conv1d_transpose(
            stft_real,
            self.idft_real,
            output_shape=[tf.shape(stft_real)[0], 
                         (tf.shape(stft_real)[1] - 1) * self.hop_length + self.n_fft, 
                         1],
        ...
        istft_imag = tf.nn.conv1d_transpose(
            stft_imag,
            self.idft_imag,
            output_shape=[tf.shape(stft_imag)[0], 
                         (tf.shape(stft_imag)[1] - 1) * self.hop_length + self.n_fft, 
                         1],
            strides=self.hop_length,
            padding='VALID'
        )
