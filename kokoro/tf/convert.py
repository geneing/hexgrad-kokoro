"""PyTorch checkpoint to TensorFlow weight conversion entrypoint.

This module provides a thin CLI around `kokoro.tf.checkpoint_utils` to convert
the trained PyTorch Vocos generator checkpoint into TensorFlow-native outputs:
- Keras model weights (`.weights.h5`)
- TensorFlow checkpoint prefix (`generator_tf_ckpt-*`)
- Inferred generator config JSON

The conversion maps tensor layouts and parameter names between frameworks and is
used by both standalone conversion workflows and downstream LiteRT export jobs.
"""
...
from .checkpoint_utils import convert_pytorch_checkpoint_to_tf
