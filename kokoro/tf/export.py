"""TensorFlow Vocos quantization/QAT/export pipeline for LiteRT.

This module orchestrates the full TensorFlow deployment flow:
- Load and map a PyTorch generator checkpoint into TensorFlow.
- Optionally run short QAT-style fine-tuning on paired training items.
- Export fp32/fp16/int8 LiteRT variants.
- Score and compare export candidates with quality and operator-coverage metrics.
- Write validation reports and generated audio artifacts.

It is intentionally "pipeline-level" code. Core model layers are defined in
`kokoro.tf.model`, while checkpoint I/O and tensor mapping are in
